...
import { DollarSign, FileText, Activity, TrendingUp, ShieldCheck, Lock, ChevronDown, ChevronRight, FilterX, Calendar, Clock, AlertTriangle, FileCheck, ArrowRight, Printer, Shield, Banknote, Tag, Building2, User, PanelLeftClose, PanelLeft } from "lucide-react";
...
import { SlicerFilter } from "@/components/slicer-filter";
...
export default function ExecutiveView() {
        ...
        <div className="pb-3 border-b bg-muted/30 rounded-t-lg px-4 py-3">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mt-1">
              ...
              {parent?.modalidadContratacion && (
                <span className="flex items-center gap-1">
                  <Tag className="h-3 w-3" />
                  Modalidad: <span className="text-foreground font-medium">{parent.modalidadContratacion}</span>
                </span>
              )}
              {parent?.contacto && (
                <span className="flex items-center gap-1">
                  <User className="h-3 w-3" />
                  Contacto: <span className="text-foreground font-medium">{parent.contacto}</span>
                </span>
              )}
            </div>
            ...
            {(() => {
              const totalMontoEjecutado = contractDetails.items.reduce((s, i) => s + i.montoEjecutado, 0);
              return totalMontoEjecutado > 0 ? (
                <div className="mt-2 flex flex-wrap gap-4 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <Banknote className="h-3 w-3" />
                    Monto Ejecutado: <span className="text-foreground font-mono font-medium">{fmt(totalMontoEjecutado)}</span>
                  </span>
                </div>
              ) : null;
            })()}
                  ...
                  <div className="border-t">
                    ...
                    {allInvoices.length === 0 ? (
                      <div className="px-4 py-3 text-xs text-muted-foreground italic">
                        No se encontraron registros de facturas. Verifique que el archivo Excel contenga la hoja "Pago", "4Pago" o "Pagos" con las columnas CONTRATO, ADENDA, VALOR SIN IGV.
                      </div>
                    ) : (
                      <div className="overflow-x-auto">
                        <Table>
  ...
  return (
    <Layout>
      {/* Two-column layout: KPIs left, Inversión chart right */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6 print:grid-cols-1">
        {/* Left Column: Title, Filters, KPIs */}
        <div className="lg:col-span-7 space-y-4">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-2 print:mb-2">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto print:hidden">
              ...
              <SlicerFilter
                label="Clase:"
                options={availableContractClasses}
                selected={selectedContractClasses}
                onSelectionChange={setSelectedContractClasses}
              />
          ...
          {selectedCategories.length > 0 && (
            <div className="flex items-center gap-2 text-xs print:text-lg text-primary font-medium print:font-bold">
        ...
        {/* Right Column: Inversión por Tipo chart */}
                        ...
                        onClick={(e) => {
                          if (e.ctrlKey || e.metaKey) {
                            ...
                            setSelectedCategories(
                              selectedCategories.length === 1 && selectedCategories[0] === entry.name ? [] : [entry.name]
                            );
                          }
                        }}
                        className={cn(
